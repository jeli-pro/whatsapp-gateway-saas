...
function getInstanceProxyUrl(instance: { id: number }, node: { publicHost: string }, subPath: string): string {
  // In test environment, we connect directly to the mapped port on localhost.
  if (process.env.NODE_ENV === 'test') {
    const port = 30000 + instance.id;
    // The `publicHost` is configured to be 'localhost' in the test setup.
  ...
  return new Elysia()
    ...
    .group('/api', (app) => app
      ...
      .get('/instances/:id/qr', async ({ params, set, user }) => {
        ...
        const instanceUrl = getInstanceProxyUrl(instanceData.instances, instanceData.nodes, '/qr');
      ...
      .post('/instances/:id/send', async ({ params, body, set, user }) => {
        ...
        const instanceUrl = getInstanceProxyUrl(instanceData.instances, instanceData.nodes, '/send');
